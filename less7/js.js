...
let renderer = {
    ...
    render(snakePointArray, foodPoint, wallPoint) {
        ...
        this.cells[`x${wallPoint.x}_y${wallPoint.y}`].classList.add('wall'); //Рендим 2 куба стены
        this.cells[`x${wallPoint.x+1}_y${wallPoint.y}`].classList.add('wall');
    }
};
...
let wall = {  //Стена аля Еда
    ...
    isWallPoint(point) {
        return (this.x === point.x && this.y === point.y) || (this.x1 === point.x && this.y === point.y) ;
    }
};
...
let game = {
    ...
    reset() {
        ...
        wall.setWallCoordinates(this.getRandomCoordinates()); //генерим корды стены

        this.render();
    },

    render() {
        this.renderer.render(this.snake.body, this.food.getFoodCoordinates(), wall.getWallCoordinates()); 
    },
    ...
    finish() {
        ...
        document.getElementById("counterWin").innerHTML='Побед - ' + this.wins; //Выводим кол-во побед
        document.getElementById("counterLost").innerHTML='Поражений - ' + this.lost;  //Выводим кол-во поражений
        console.log('Побед - ' + this.wins);
    },

    tickHandler() {
        if (!this.canSnakeMakeStep()) {
            this.lost++; //Считаем поражения об себя и о главные стены
            this.finish();
            return;
        }

        if (wall.isWallPoint(this.snake.getNextStepHeadPoint())) {  //Если стенка то проиграть
            this.lost++; //Считаем поражения от столкновения со стенкой
            this.finish();
            return;            
        }
        ...
        if (this.food.isFoodPoint(this.snake.getNextStepHeadPoint())) {
            ...
            this.food.setFoodCoordinates(this.getRandomCoordinates());
            wall.setWallCoordinates(this.getRandomCoordinates()); //Обновить стену
            if (this.isGameWon()) {
                this.wins++; //Считаем победы
                this.finish();
            }
        }
...
console.log(wall.getWallCoordinates());
